...
TEST(ParseCapabilities, WithAndroidPackage) {
  ...
  caps.SetString("xwalkOptions.androidPackage", "abc");
...
TEST(ParseCapabilities, EmptyAndroidPackage) {
  Capabilities capabilities;
  base::DictionaryValue caps;
  caps.SetString("xwalkOptions.androidPackage", std::string());
  Status status = capabilities.Parse(caps);
...
TEST(ParseCapabilities, IllegalAndroidPackage) {
  Capabilities capabilities;
  base::DictionaryValue caps;
  caps.SetInteger("xwalkOptions.androidPackage", 123);
  Status status = capabilities.Parse(caps);
  ASSERT_FALSE(status.IsOk());
}
...
TEST(ParseCapabilities, LogPath) {
  Capabilities capabilities;
  base::DictionaryValue caps;
  caps.SetString("xwalkOptions.logPath", "path/to/logfile");
  Status status = capabilities.Parse(caps);
  ASSERT_TRUE(status.IsOk());
...
TEST(ParseCapabilities, Args) {
  ...
  base::DictionaryValue caps;
  caps.Set("xwalkOptions.args", args.DeepCopy());
...
TEST(ParseCapabilities, Prefs) {
  ...
  base::DictionaryValue caps;
  caps.Set("xwalkOptions.prefs", prefs.DeepCopy());
  Status status = capabilities.Parse(caps);
  ASSERT_TRUE(status.IsOk());
...
TEST(ParseCapabilities, LocalState) {
  ...
  base::DictionaryValue caps;
  caps.Set("xwalkOptions.localState", local_state.DeepCopy());
  Status status = capabilities.Parse(caps);
  ASSERT_TRUE(status.IsOk());
...
TEST(ParseCapabilities, Extensions) {
  ...
  base::DictionaryValue caps;
  caps.Set("xwalkOptions.extensions", extensions.DeepCopy());
  Status status = capabilities.Parse(caps);
  ASSERT_TRUE(status.IsOk());
...
TEST(ParseCapabilities, ExcludeSwitches) {
  ...
  base::DictionaryValue caps;
  caps.Set("xwalkOptions.excludeSwitches", exclude_switches.DeepCopy());
  Status status = capabilities.Parse(caps);
  ASSERT_TRUE(status.IsOk());
